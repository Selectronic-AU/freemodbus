...
xMBMasterPortSerialInit( UCHAR ucPORT, ULONG ulBaudRate, UCHAR ucDataBits, eMBParity eParity )
{
    ...
    /* open serial device */
    if( !serial->parent.open( &serial->parent, RT_DEVICE_OFLAG_RDWR | RT_DEVICE_FLAG_INT_RX ) )
    {
    ...
    rt_thread_init( &thread_serial_soft_trans_irq, "master trans", serial_soft_trans_irq, RT_NULL,
                    serial_soft_trans_irq_stack, sizeof( serial_soft_trans_irq_stack ), 10, 5 );
...
vMBMasterPortSerialEnable( BOOL xRxEnable, BOOL xTxEnable )
{
    ...
    else
    {
        ...
        rt_event_recv( &event_serial, EVENT_SERIAL_TRANS_START, RT_EVENT_FLAG_OR | RT_EVENT_FLAG_CLEAR, 0,
                       &recved_event );
    }
}
...
serial_soft_trans_irq( void *parameter )
{
    ...
    while( 1 )
    {
        ...
        rt_event_recv( &event_serial, EVENT_SERIAL_TRANS_START, RT_EVENT_FLAG_OR, RT_WAITING_FOREVER, &recved_event );
