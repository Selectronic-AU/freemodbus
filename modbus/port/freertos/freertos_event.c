...
// Keep the static queue buffer naturally aligned.
static eMBEventType mb_event_queue_data[MB_SLAVE_FREERTOS_EVENT_QUEUE_LENGTH];

/// @brief Initialize the event queue for the FreeModbus stack.
BOOL
xMBPortEventInit( void )
{
    mb_event_queue_handle = xQueueCreateStatic( mb_event_queue_length, mb_event_queue_size,
                                                ( uint8_t * ) mb_event_queue_data, &mb_event_queue );
...
// Keep the static queue buffer naturally aligned without requiring C11 alignas.
static eMBMasterEventType mb_master_event_queue_data[MB_MASTER_FREERTOS_EVENT_QUEUE_LENGTH];
...
{
    mb_master_event_queue_handle =
        xQueueCreateStatic( mb_master_event_queue_length, mb_master_event_queue_size,
                            ( uint8_t * ) mb_master_event_queue_data, &mb_master_event_queue );
